...
/* Ludo: Table inserted for speedup of bitcounting, that is done frequently in obtain_nswitchings() */
unsigned int bits_in_char[256] =
...
/*		tbus = *(ibus + i) ^ *(jbus + i);
		...
		tbus = *(ibus + i) ^ *(jbus + i);
		nswitchings += bits_in_char[tbus];
		/* Ludo: old code, changed into a table lookup */ 
	}
	return nswitchings;
}
