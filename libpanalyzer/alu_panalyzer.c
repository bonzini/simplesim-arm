/*
* alu_panalyzer.c - alu prediction power model 
* 
*
...
/* create level 1 power model alu panalyzer database
 * return an allocated location pointer.
 * caution: please deallocate the lv1_alu space  */

fu_alu_pspec_t *
create_alu_panalyzer(
	char *name, /* lv1_alu name */
	int freq,double volt,
	double iCeff)
{
	fu_alu_pspec_t *pspec;

	fprintf(stderr, "\n****************************************************\n");
	fprintf(stderr, "\t\t power model alu panalyzer configuration\n");
	fprintf(stderr,   "****************************************************\n");
	if(!(pspec = (fu_alu_pspec_t *)malloc(sizeof(fu_alu_pspec_t))))
	...
	)
{
	...
	pspec->alu_total_power += dynamic_power; 
	pspec->alu_access++;
	pspec->alu_power += dynamic_power;
	pspec->max_power = 
		(pspec->max_power > dynamic_power) ? pspec->max_power:dynamic_power; 
	pspec->avg_power = pspec->alu_total_power / (double) now;
}
