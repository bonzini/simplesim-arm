/*
* fpu_panalyzer.c - branch prediction power model 
* 
*
	...
	double iCeff)
{
	...
	fprintf(stderr, "\n****************************************************\n");
	fprintf(stderr, "\t\t power model fpu panalyzer configuration\n");
	fprintf(stderr,   "****************************************************\n");
	if(!(pspec = (fu_fpu_pspec_t *)malloc(sizeof(fu_fpu_pspec_t))))
	...
	)
{
	...
	pspec->pipe_fpu_power[0] =  
			pspec->pipe_fpu_power[1] =  
			pspec->pipe_fpu_power[2] = dynamic_power / 3;  
	
	pspec->avg_power = pspec->fpu_total_power / (double) now;
}
