	...
	double sCeff, double iCeff, double lCeff)
{
	...
	pspec->pmodel = pmodel; 
	pspec->opfreq = opfreq; 
	pspec->svolt = Vio; /* io supply voltage */
	...
	if(pmodel == Analytical) 
	{
		...
		btr_opt = estimate_btr_opt(nbstages_opt, sCeff + packageCeff);
	fprintf(stderr, "optimal buffer transistor ratio: %3.4e\n", btr_opt);
		pspec->Ceffs->sCeff = sCeff + packageCeff /* (packaging cap) */;
		pspec->Ceffs->iCeff = estimate_ibufchainiCeff(btr_opt, sCeff + packageCeff);
		pspec->Ceffs->lCeff 
