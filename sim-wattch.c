...
struct RUU_station {
  ...
  quad_t val_in[MAX_IDEPS], val_out[MAX_ODEPS];
  ...
  int inames[MAX_IDEPS];		/* input logical names (NA=unused) */
...
/* CHECK */
          ...
          if ((MD_OP_FLAGS(rs->op) & (F_ICOMP|F_FCOMP))
	      || ((MD_OP_FLAGS(rs->op) & (F_MEM|F_LOAD)) == (F_MEM|F_LOAD)))
	    {
	      int i;
	      for (i = 0; i < MAX_ODEPS; i++)
	        if(rs->onames[i] != NA)
...
#ifdef DYNAMIC_AF	
		    regfile_total_pop_count_cycle += pop_count(rs->val_out[i]);
  ...
  while ((rs = eventq_next_event()))
      ...
      if(!(MD_OP_FLAGS(rs->op) & F_CTRL)) {
	{
	  ...
	  for (i = 0; i < MAX_ODEPS; i++)
		...
		resultbus_access++;
#ifdef DYNAMIC_AF	
		window_total_pop_count_cycle += pop_count(rs->val_out[i]);
		window_num_pop_count_cycle++;
		resultbus_total_pop_count_cycle += pop_count(rs->val_out[i]);
      ...
      if (RSLINK_VALID(node))
	{
	  ...
	  if (rs->in_LSQ
		...
		panic("mis-predicted store");
	      ...
	      lsq_preg_access++;
#ifdef DYNAMIC_AF	
	      lsq_total_pop_count_cycle += pop_count(rs->val_in[0]);
...
#ifdef DYNAMIC_AF	
				      lsq_total_pop_count_cycle += pop_count(rs->val_out[0]);
				      lsq_num_pop_count_cycle++;
#endif
				      break;
				    }
			  ...
			  if(rs->inames[i] != NA)
			    {
			      ...
			      window_preg_access++;
#ifdef DYNAMIC_AF	
			      window_total_pop_count_cycle += pop_count(rs->val_in[i]);
			      window_num_pop_count_cycle++;
#endif
			    }
		      }
  ...
  if (!head.rs)
    {
      ...
      regfile_access++;
#ifdef DYNAMIC_AF	
      regfile_total_pop_count_cycle += pop_count(rs->val_in[idep_num]);
      regfile_num_pop_count_cycle++;
#endif
...
ruu_dispatch(void)
{
  ...
  static struct bpred_update_t *dir_update_ptr;/* branch predictor dir update ptr */
...
#include "machine.def"
	default:
      ...
      val_in[0] = (in1 != NA ? GPR(in1) : 0);
      val_in[1] = (in2 != NA ? GPR(in2) : 0);
      val_in[2] = (in3 != NA ? GPR(in3) : 0);
      val_in[3] = (in4 != NA ? GPR(in4) : 0);
      ...
      if (verbose && sim_num_insn >= trigger_inst)
	{
      ...
      val_out[0] = (out1 != NA ? GPR(out1) : 0);
      val_out[1] = (out2 != NA ? GPR(out2) : 0);
      val_out[2] = (out3 != NA ? GPR(out3) : 0);
      ...
      if (op != MD_NOP_OP)
	{
	  ...
	  if (!(MD_OP_FLAGS(op) & F_MEM))
		...
		(op == AGEN || op == AGEN_U || op == AGENI || op == AGENI_U);
	      ...
	      rs->val_in[0] = val_in[0];
	      rs->val_in[1] = val_in[1];
	      rs->val_in[2] = val_in[2];
	      rs->val_in[3] = val_in[3];
	      rs->val_out[0] = val_out[0];
	      rs->val_out[1] = val_out[1];
	      rs->val_out[2] = val_out[2];
	      ...
	      {
		int i;
		/* Wattch -- for each operand ready, 1 window write access */
		for (i = 0; i < MAX_IDEPS; i++)
		  if(rs->idep_ready[i])
		    {
		      window_access++;
...
#ifdef DYNAMIC_AF	
		      regfile_total_pop_count_cycle += pop_count(rs->val_in[i]);
		      regfile_num_pop_count_cycle++;
#endif
		    }
	      }
	  ...
	  else
	    {
	      ...
	      */
	      ...
	      /* Wattch: Maintain values through core for AFs*/
	      lsq->val_in[0] = val_in[0];
	      lsq->val_in[1] = val_in[1];
	      lsq->val_in[2] = val_in[2];
	      lsq->val_in[3] = val_in[3];
	      lsq->val_out[0] = val_out[0];
	      lsq->val_out[1] = val_out[1];
	      lsq->val_out[2] = val_out[2];
